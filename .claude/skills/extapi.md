...
Every service includes a `POST /{service}/passthrough` endpoint for arbitrary API calls not covered by the named endpoints.

## Base URL

```
http://127.0.0.1:11583
```
...
```json
{
  ...
  "path": "/api/path/here",
  "body": { ... },       // optional, omit for GET/DELETE
  "params": { "key": "value" }  // optional query params
}
```

The `path` is relative to the service's upstream base URL. Non-GET methods are logged as mutations.
